...
async function getTournamentData(tournamentId: string, userId: string) {
  ...
  // Members with names
  const memberIds = (tournament.members as Types.ObjectId[]).map((m) => new Types.ObjectId(m.toString()));
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <div className="flex items-center gap-1.5 rounded-md border border-zinc-700 px-3 py-1.5 text-sm font-mono text-zinc-300">
            ...
            <span className="font-semibold tracking-widest">{tournament.inviteCode}</span>
          </div>
          {nextGP && (
            <Link
          ...
          <Table>
            <TableHeader>
              <TableRow className="border-zinc-800 hover:bg-transparent">
                ...
                <TableHead className="text-zinc-400 text-right">Puntos</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {leaderboard.map((row) => (
                ...
                >
                  <TableCell className="text-zinc-400 font-mono">{row.rank}</TableCell>
                  ...
                  <TableCell className="text-zinc-400 text-right">{row.gpsPlayed}</TableCell>
                  <TableCell className="text-right font-bold text-white">{row.total}</TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
